...
def import_teams_to_db(ranking:dict):
    for i in ranking.keys():
        query = insert(teams_table).values(id=i, name=ranking[i]['name'],
...
def import_bets_teams_to_db(username: str, bets: dict):
    for i in bets.keys():
        ...
        query = update(users_table).where(users_table.c.id==1).values(
                                                team1ID=team1ID, team2ID=team2ID,
                                                team3ID=team3ID, team4ID=team4ID,
                                                team5ID=team5ID, team6ID=team6ID,
                                                more_goals_for=more_goals1,
                                                less_goals_for=less_goals1,
                                                more_goals_against=more_goals2,
                                                less_goals_against=less_goals2,
                                                winner=winner, more_goals_total=more_goals3)
        with engine.begin() as conn:
            conn.execute(query) 